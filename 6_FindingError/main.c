...
int main()
{
  //  multiple_input_multiple_output_nn(inputs,IN_LEN,predicted_result,OUT_LEN,weigths);
    ...
    printf("Sad error is %f \r\n",find_error_simple(predicted_result[SAD_PREDICTION_IDX],expected_values[SAD_PREDICTION_IDX]));
    printf("Sick prediction is %f \r\n",predicted_result[SICK_PREDICTION_IDX]);
    printf("Sick error is %f \r\n",find_error_simple(predicted_result[SICK_PREDICTION_IDX],expected_values[SICK_PREDICTION_IDX]));
    printf("Active prediction is %f \r\n",predicted_result[ACTIVE_PREDICTION_IDX]);
    printf("Active error is %f \r\n",find_error_simple(predicted_result[ACTIVE_PREDICTION_IDX],expected_values[ACTIVE_PREDICTION_IDX]));
    return 0;
}
